 ...
 */
    ...
    var mainHtmls = {};
    var blankRegs = [];
    var target='http://www.hzti.com';
    var cacheRegex = /(\/image\/|\/images\/|\.css$|\.js$)/i
    var ignoreRegex
    blankRegs.push(/<script[^>]*>[^<>]+baidu\.com[^<>]+<\/script>/gmi);
    blankRegs.push(/onload="rand\(\);"/i);
    var injs = autoProxy.getInjectedHtml()
        + ' <script type="text/javascript" src="/zj/injected-10580.js"></script>';

    var mainHtmlArray=[];
    var url = '/service/qry/violation_veh.aspx?type=2&node=249';
    var mainHtml = {url: url,
        origin: saveFileName.origin( 'origin_10571_vio_query.html'),
        overload: saveFileName.overload('overload_10571_vio_query.html'),
    ...
    mainHtml =  {url: url, origin: saveFileName.origin( 'origin_10571_vio_save.html'),
